...
Phaser 3 + React + Vite + TypeScript “card shop management / idle” foundation with an **engine-agnostic sim core** in `src/sim/**`.
...
- Dev server binds to **port 5173** with `strictPort: true`.
...
- For file-based preview, build and use `dist/` as the preview root:
...
Unit tests cover sim clock transitions, speed upgrade rules, and XP leveling (no Phaser).

## Architecture

- **Sim (rules, pure TS)**: `src/sim/**`
- **Phaser (render + input + sim ticking)**: `src/game/**`
- **React (UI + dispatch via EventBus facade)**: `src/App.tsx`, `src/game/GameFacade.ts`
...
## TODO roadmap (next milestones)

See `docs/GAME_DESIGN.md`, but the near-term priorities are:

- Packs + card acquisition loop
- Deeper battle system + battle UI
- More skills and unlock-gated upgrades (decor, shop size, spawn tuning)
- Balance pass across days / tiers

## Template credit

Bootstrapped from `phaserjs/template-react-ts` (MIT).

<details>
<summary>Archived template notes (expand)</summary>
...
Phaser 3 + React + Vite + TypeScript project evolving into a **Card Shop Management Simulator / Idle Game**.
...
<!--
Archived template README content below (kept for reference; not rendered).
...
- **Customers** spawn day+night (night slower), buy for money+XP, or become battle-ready
- **Pause/resume** (freezes sim time)
- **XP → levels → skill points**
- **Unlock gate**: skill to unlock **tier 2 cards**
- **Speed modulator upgrade**: +0.05 per tier, cap 3.0x, monotonic costs, affects sim speed
- **Deck editor** (add/remove cards with tier gating)
- **Battles** against battle-ready customers (instant MVP resolver)
- **Persistence**: localStorage save/load + schema versioning; autosave on night start; manual save/load
